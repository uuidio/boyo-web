<template>
    <div class="shop_list">
        ...
        <div class="add_shop">
            ...
            <draggable :options="sortOptions"   v-model="dataLists" style="position: relative">
             <div class="card_item item" v-for="(item,index) in dataLists" :key="index">
                  ...
                  <div class="card_context">
                       ...
                       <div class="card_href">
                           <span>链接：</span>
                           <urlSelect @on-change="getChangeItem" :init_level="item.level" :url_type="item.urlType" :init_value="item.value">
          ...
          <div class="add_card_list" @click="addCardList" v-if="!config.is_mini">
              <a-icon type="plus" />
              添加一个图文导航
          </div>
        <div class="mt20">
          <span class="mr10">一行展示个数:</span>
          <a-radio-group name="radioGroup" v-model="config.isNumber">
      ...
      <div class="mt20">
        <div class="mb10">上下内边距：</div>
        <div>
          <a-slider :min="0"  v-model="config.p1" />
        </div>
      </div>
      <div class="mt20">
        <div class="mb10">左右外边距：</div>
        <div>
          <a-slider :min="0"  v-model="config.p2" />
        </div>
      </div>
      <div class="mt20">
        <div class="mb10">边框圆角：</div>
        <div>
...
export default {
  ...
  methods: {
    ...
    itemDel(index) {
      const urlList = this.shop_list[this.editIndex].options;
