...
<!--              <a-select show-search :filter-option="SltFilterOption"-->
<!--                        v-model="formItem.corp_code" style="width:300px">-->
<!--                <a-select-option-->
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      addLogistics: "trade/addLogistics"
    }),
    onChange(value) {
    ...
    searchValue(value) {
      let data = this.logistics.filter(item => item.corp_name.indexOf(value) > -1);
      if (data.length > 100 || value === '') {
        data = data.slice(0, 100);
      }
      this.optionData = data;
    },
    loadOption(pageIndex) {
      const newPageSize = this.pageSize * (pageIndex || 1);
      let newOptionsData = [], len;
      if (this.logistics.length > newPageSize) {
        len = newPageSize;
      } else {
        len = this.logistics.length;
      }
      if (this.keyWords) {
        const data = this.logistics.filter(item => item.corp_name.indexOf(this.keyWords) > -1) || [];
      ...
      this.optionData = newOptionsData;
    },
    handleScroll(e) {
      // e.persist();
      const { target } = e;
      ...
      if (rmHeight === 0 && clHeight === 0) {
        ...
        if (rmHeight < clHeight + 5) {
          ...
          this.loadOption(this.scrollPage + 1);
        }
      }
  ...
  watch: {
    logistics() {
      this.initData = this.logistics.slice(0, 100);
      this.optionData = this.logistics.slice(0, 100);
    },
    detail() {
      this.initDet();
    },
