<template>
  <div class="goodsList" @click="getLists">
    ...
    >
      ...
      <a-table :scroll="{ x: 1100,y:400 }" :rowKey="record => record.id"  :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}" :loading="tabLoading" :columns="listField" :dataSource="listData" class="components-table-demo-nested" :pagination="false">
        ...
        for(let i = 0;i < items.length;i++){
          if(this.ids.indexOf(items[i].id)===-1){
            ...
            if (this.selectType==='shop') {
              itemdata.shop_name = items[i].shop_name;
            }else{
              itemdata.gc_name = items[i].gc_name;
            }
            this.selectData.push(itemdata)
