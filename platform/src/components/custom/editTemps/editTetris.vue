<template>
    <div class="shop_list">
        ...
        <div class="page_group page_back">
            ...
            <div class="add_cart_item" v-for="(item,index) in options" :key="index">
               <div class="cart_tim">
                  <div class="cart_left">
                     ...
                     <span class="modify-image" @click="changeImg(index)">更换图片</span>
                  </div>
                  <div class="cart_right">
                    ...
                    <div class="cart_link">
                       链接<div class="cart_link-item">
						    <urlSelect :fit_up="config.isFitUp" :url_type="item.link_type" :init_value="item.link_value"  @on-change="getChangeItem" :fx="true">
            ...
            <div class="add_shop" v-if="options.length<4">
        ...
        <!-- <div class="page_group page_back" v-else>
            <div class="add_cart_item" v-for="(item,index) in config.sub_entry" :key="index">
               <div class="cart_tim">
                  <div class="cart_right">
                    <div class="cart_title">
                       标题<input type="text" v-model="item.title"/>
                    ...
                    <div class="cart_link">
                       链接<div class="cart_link-item">选择跳转到的页面</div>
                    </div>
                  </div>
                  <div class="cart_del" @click="delSubEntry(index)">
                     x
            ...
            <div class="add_shop">
                <div class="add_card_list" @click="addtextList">
                  <a-icon type="plus" />
                   添加图文导航
                </div>
            </div>
        ...
        <div class="page_group"> 
             <div class="add_contain">
                <div class="add_title">
                   ...
                   <div class="add_value">{{config.template==0?'模式1':config.template==1?'模式2':config.template==2?'模式3':'模式4'}}</div>
                </div>
                <div class="add_action">
				  ...
				  <span :class="[config.template=='3'?'action_border':'']" @click="changeStyle('3')"><a-icon type="align-center" /></span>
                </div>
             </div>
          </div>
		  <div class="page_group"> 
                  ...
                  <div class="add_slide">
                     <a-row class="shop_margin_ant-row">
                      <a-col class="shop_margin_rang">
                        <a-slider :min="0" :max="10" v-model="config.md" />
                      </a-col>     
                     </a-row>
        ...
        <!-- <div class="page_group"> 
             <div class="add_contain">
                <div class="add_title">
                   <div class="add_lable">背景颜色</div>
                </div>
                <div class="add_action">
                     <a href="#" style="color: #38f;text-decoration: none;cursor: pointer;margin-right:10px" @click="backBackColor">重置</a>
                    <input type="color" v-model="config.background_color"/>
                </div>
             </div>
          </div>
          <div class="page_group"> 
             <div class="add_contain">
                <div class="add_title">
                   <div class="add_lable">文字颜色</div>
                </div>
                <div class="add_action">
                     <a href="#" style="color: #38f;text-decoration: none;cursor: pointer;margin-right:10px" @click="backColor">重置</a>
...
export default {
  ...
  methods:{
    ...mapActions({
     ...
     initData() {
	//   this.config=this.shop_list[this.editIndex];
	//   console.log('组件',this.config);
	},
	 getChangeItem(item){
		console.log('选中',item);
		this.options[this.selectIndex].link_value=item.value;
		this.options[this.selectIndex].link_type=item.urlType;
	  },
	getIndex(index){
    ...
    showModel(){
     ...
     handleOk() {
		...
		if(this.is_change){
				this.options[this.selectIndex].image_url=imgs[0].url
		}else{
			const obj = {
				link_type: "",
				link_value: "",
			...
			//this.shop_list[this.editIndex].options.push(obj);
		}
      	this.visible = false;
    ...
    changeImg(index){
        ...
        this.visible=true
		//this.imageLists=[]
		//this.$refs.uploadImageLists.uploadImageList=[]
		//this.$refs.uploadImageLists.defaultImageList=[]
    },
    changeStyle(val){
	  this.config.template=val;
	  //this.shop_list[this.editIndex].config.template=val;
    },
    onChange(e){
  ...
  watch:{
    // config(val){
    //     this.shop_list[this.editIndex]=val
	// },
  }
}
</script>
