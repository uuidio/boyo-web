    ...
    <a-modal width="80%" :visible="isOpenModal" :closable="false" :maskClosable="false" @ok="handleOk" @cancel="handleCancel" :title="'添加' + modelName">
      <a-table :scroll="{ x: 1100,y:700 }" :rowKey="record => record.id" :columns="listField" :dataSource="listData" :pagination="false">
       <div slot="image_url" slot-scope="image_url,item">
        ...
        <div  slot="value" slot-scope="text, item">{{ item.value.length }}个商品</div>
      ...
      <a-form :label-width="80">
        ...
        <div v-if="type === 'show_list'">
          <a-form-item label="展示数据">
            ...
            <TableSelect :goodId="value" :goods-max="goodsMax" selectionRef="checkbox" showType="goods" :selectionUrl="selectionGoodsUrl" @on-select-ok="selectedGoodsList"
...
import { shopType, imageSiteConfigField, listsSiteConfigField, goodsSwiperHead ,recommendSiteConfigField} from '@/config/site';
      ...
      const actionCol = [
        {
          ...
          this.listField = getUnion(recommendSiteConfigField, actionCol);
          this.listField[2].title='热卖商品已选数量'
          this.listField[2].scopedSlots= { customRender: 'value' }
