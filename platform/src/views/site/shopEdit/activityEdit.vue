<template>
  <div class="content ">
    <div class="titleConfig mb20">
      页面标题设置：
      <a-input style="width: 50%;" v-model="headerTitle" placeholder="请输入页面标题" @change="changeHeaderTit"/>
    </div>
    <div class="editHome">
      ...
      <div class="edit_Box" v-if="items.edit">
        <component :is="'edit'+items.id" editType="activity" :editIndex="items.index"></component>
      </div>
    </div>
...
<script>
  ...
  import { Swipers, editSwipers, GrapNav, editGrapNav, goodGroup, editgoodGroup, CustomImg, editCustomImg, Title, editTitle, Navs, editNavs } from '../shopConfig/shopConfig';
  ...
  export default {
    computed: {
      shop_list: {
        ...
        set(val) {
          this.$store.state.custom.activityItems = val;
        },
      },
      ...mapState({
        customItemsId: state => state.custom.customItemsId,
      }),
    },
    components: {
      draggable,
    ...
    methods: {
      ...mapActions({
        setActivityItems: 'custom/setActivityItems',
        getActivityItems: 'custom/getActivityItems',
        cleanActivityItems: 'custom/cleanActivityItems',
      }),
      changeHeaderTit(e) {
        this.shop_list[0].title = e.target.value;
      },
      start(e) {
      },
      callback() {
        console.log(this.customItemsId);
        this.custom_id = this.customItemsId.custom_id;
      },
      submit() {
        if (this.headerTitle === '') {
    ...
    mounted() {
      if (this.custom_id) {
        this.getActivityItems({ page: 'custom_activity', custom_id: this.custom_id });
      } else {
        this.cleanActivityItems();
      }
    },
  };
</script>
