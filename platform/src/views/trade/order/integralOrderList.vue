<template>
  <div class="content">
    ...
    <searchItem class="mb20" @on-search="getSearch" :showSltActivity="false"></searchItem>
    ...
    <template v-if="listField&&listField.length>0">
      <a-tabs @tabClick="onGetData" :defaultActiveKey="trim">
        <a-tab-pane v-for="(item, index) in tabLists" :tab="item.tab" :key="item.key">
          <a-table :scroll="{ x: 1800,y:700 }"  :rowKey="record => record.id" :rowSelection="{onChange: onChange}" :columns="listField" :dataSource="listData" :loading="loadingState" :pagination="false">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        listData: state => state.trade.integralOrderLists,
        listField: state => state.trade.integralOrderFields,
        pages: state => state.trade.integralOrderPages,
		exportData: state => state.trade.exportIntegralOrderData,
    ...
    methods: {
      ...mapActions({
        fetchLists: 'trade/integralOrderLists',
        cancelOrder: 'trade/cancelOrder',
        exportFilter: 'trade/exportIntegralGood',
      }),
      getLists(){
      ...
      export2Excel(tHeaders, filterVals, lists) {
        ...
        import("@/vendor/Export2Excel").then(excel => {
          excel.export_json_to_excel({
            ...
            data, //具体数据 必填
            filename: this.UserType === 'normal' ? "品牌积分订单管理列表" : '品牌牛币订单管理列表', //非必填
            autoWidth: true, //非必填
            bookType: "xlsx" //非必填
          });
        });
