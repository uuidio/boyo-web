  ...
  <div class="mr24">
    <a-card :bordered="false" title="订单进度">
      <div class="steps">
        ...
        <div class="fahuo" v-if="$route.query.type">
          <a-button
            type="primary"
            @click="goDetail(detail.tid)"
            v-if="detail.status_text == '待发货' && detail.pick_type === 0"
          >
            我要发货
          </a-button>
          <a-button
            type="primary"
            @click="pickUp(detail)"
            v-if="detail.status_text == '待发货' && detail.pick_type === 1"
          >
            我要提货
          </a-button>
        </div>
    ...
    <a-card :bordered="false" title="订单信息">
      ...
      <div class="det-info">
        <a-row>
          <a-col :md="6">
            <p v-if="detail.shop_id != '0'">订单店铺： {{ detail.shop_info.shop_name }}</p>
            <p v-if="detail.shop_id == '0'">品牌活动订单</p>
            <p>订单号：{{ detail.tid }}</p>
          ...
          <a-col :md="6">
            ...
            <p>消耗{{detail.point_unit}}：{{detail.consume_point_fee}}</p>
