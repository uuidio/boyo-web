...
export default {
  ...
  created() {
    if (this.$route.query.activityId) {
      ...
      this.getArticleItem(this.activityId);
    } else {
      // this.initDataItem();
    }
  ...
  methods: {
    ...mapActions({
      addDateItem: 'member/addActivity',
    ...
    initData() {
      if (this.dataItem.activity_url) {
        this.imageLists = [{ url: this.dataItem.activity_url, status: 'finished' }];
      }
    ...
    submitData() {
      let obj = {
        ...
        activity_url: this.formItem.activity_url,
      ...
      if (this.$refs.uploadImageLists.uploadImageList.length > 0) {
        obj.activity_url = this.$refs.uploadImageLists.uploadImageList[0].url;
      }
