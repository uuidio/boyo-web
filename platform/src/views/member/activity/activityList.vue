<template>
  <div class="content">
    <div class="list-nav">
      <div class="mb10">
        <router-link to="/member/addActivity">
          <a-button class="mr10" type="primary" icon="plus">添加活动</a-button>
        </router-link>
      </div>
    </div>
    <div>
...
export default {
  ...
  computed: {
    ...mapState({
      listData: state => state.member.activityList,
      listField: state => state.member.activityField,
      pages: state => state.member.activityPages,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...mapActions({
      getDateList: 'member/getActivityList',
    ...
    // 编辑
    edit(item) {
      const goLink = `/member/addActivity?activityId=${item.id}`;
      this.$router.push({
        path: goLink,
      });
    },
