...
const actions = {
  ...
  exportIntegralOrder({ commit }, exportForm) { // callback
    fetch
      .get(Server.action.export_integral_Order, exportForm)
      .then((response) => {
        console.log(response);
        if (response.data.code === 0) {
          commit('setExportIntegralOrderData', response.data.result);
          // callback && callback()
        } else {
          helper.showError(response.data.message);
        }
  ...
  exportIntegralGood({ commit }, exportForm) { // callback
    fetch
      .post(Server.action.export_integral_good, exportForm)
      .then((response) => {
        console.log(response);
        if (response.data.code === 0) {
          commit('setExportIntegralOrderData', response.data.result);
  ...
  integralOrderLists({ commit }, page) {
    http
      .get(Server.action.integral_order_lists, page)
      .then((response) => {
        if (response.code === 0) {
          ...
          commit('setIntegralOrderLists', response.result.lists.data);
          commit('setIntegralOrderFields', field);
          commit('setIntegralOrderPages', response.result.lists);
  ...
  // 线下退款信息
  getOffLineInfo({ commit }, param) {
    http.get(Server.action.onceRefundDetail, param).then((res) => {
      if (res.code === 0) {
        commit('setOffLineInfo', res.result);
        console.log(res);
      } else {
        helper.showError(res.message);
      }
    });
  },
  // 物流列表
...
const mutations = {
  setExportIntegralOrderData(state, lists) { // 导出积分订单
    state.exportIntegralOrderData = lists;
  },
  setIntegralOrderLists(state, lists) { // 积分订单
    state.integralOrderLists = lists;
  },
  setIntegralOrderPages(state, lists) {
    state.integralOrderPages = lists;
  },
  setIntegralOrderFields(state, fields) {
    state.integralOrderFields = fields;
  },
  ...
  setOffLineInfo(state, data) {
    state.offLineInfo = data;
  },
