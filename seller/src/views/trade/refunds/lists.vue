...
<!--          <div v-if="item.status == '3' || item.status == '5' || item.status == '6'" slot="action" slot-scope="text, item">-->
<!--            <a href="javascript:;" @click="edit(item)">退款</a><br />-->
<!--            <a href="javascript:;" v-if="item.status == '3'" @click="rejectRefund(item)">驳回退款</a>-->
<!--          </div>-->
...
<!--              >退款信息</a>-->
<!--          </div>-->
<!--          <div v-if="item.status == '2'">已驳回</div>-->
        </a-table>
      </a-tab-pane>
    </a-tabs>
    ...
    >
      <a-form layout="horizontal">
        ...
        >
           <a-select v-model="exportForm.shop_id" placeholder="请选择门店"  >
    ...
    <a-modal centered title="退款操作" v-model="editVisible" @ok="editHandleOk">
      <a-form>
        <a-form-item
          label="支付单号"
          :required="true"
          ...
          :wrapper-col="{ md: 18, xs: 24 }"
        >
          <!--<a-input v-model="fromitem.payment_id"/>-->
          {{ fromitem.payment_id }}
        </a-form-item>
        <a-form-item
          label="应退金额"
          ...
          :wrapper-col="{ md: 18, xs: 24 }"
        >
          <!--<a-input v-model="fromitem.total_price"/>-->
        ...
        <a-form-item
          ...
          :wrapper-col="{ md: 18, xs: 24 }"
        >
          <!--<a-input v-model="fromitem.money"/>-->
          {{ fromitem.money }}
        </a-form-item>
        <div class="content-label">
          <p>说明：</p>
          <p>售后订单类型：</p>
          <p>应退金额：为订单的实际支付金额，不包括邮费</p>
          <p>实退金额：为门店输入的退款金额为准</p>
          <p>取消订单类型：</p>
          <p>按订单实际支付的金额退款（包括邮费），不存在门店输入退款金额</p>
        </div>
        ...
        <a-form-item
          ...
          :wrapper-col="{ md: 18, xs: 24 }"
        >
          <a-radio-group v-model="fromitem.refund_type">
            <a-radio value="offline">线下</a-radio>
            <a-radio value="online">原路退款</a-radio>
          </a-radio-group>
        </a-form-item>
        <template v-if="fromitem.refund_type === 'offline'">
          <a-form-item
            label="退款银行"
            :required="true"
            :label-col="{ md: 6, xs: 24 }"
            :wrapper-col="{ md: 18, xs: 24 }"
          >
            <a-input v-model="fromitem.refund_bank" />
          </a-form-item>
          <a-form-item
            label="收款银行"
            :required="true"
            ...
            :wrapper-col="{ md: 18, xs: 24 }"
          >
            <a-input v-model="fromitem.receive_bank" />
          </a-form-item>
          <a-form-item
            label="退款帐号"
            :required="true"
            ...
            :wrapper-col="{ md: 18, xs: 24 }"
          >
            <a-input v-model="fromitem.refund_account" />
          </a-form-item>
          <a-form-item
            label="收款帐号"
            :required="true"
            ...
            :wrapper-col="{ md: 18, xs: 24 }"
          >
            <a-input v-model="fromitem.receive_account" />
          </a-form-item>
          <a-form-item
            label="退款人姓名"
            :required="true"
            ...
            :wrapper-col="{ md: 18, xs: 24 }"
          >
            <!-- <a-input v-model="fromitem.refund_people" /> -->
          </a-form-item>
          <a-form-item
            label="收款人姓名"
            :required="true"
            ...
            :wrapper-col="{ md: 18, xs: 24 }"
          >
            <a-input v-model="fromitem.beneficiary" />
          </a-form-item>
        </template>
      </a-form>
    </a-modal>
    <a-modal v-model="showInfo" title="退款信息" @ok="showInfo = false">
      <a-row class="mb5">
        <a-col :span="10">退款人姓名：</a-col>
        <a-col :span="12">{{ offLineInfo.refund_people }}</a-col>
      </a-row>
      <a-row class="mb5">
        <a-col :span="10">退款订单号：</a-col>
        <a-col :span="12">{{ offLineInfo.tid }}</a-col>
      </a-row>
      <a-row class="mb5">
        <a-col :span="10">退款金额：</a-col>
        <a-col :span="12">{{ offLineInfo.money }}</a-col>
      </a-row>
      <a-row class="mb5">
        <a-col :span="10">退款银行：</a-col>
        <a-col :span="12">{{ offLineInfo.refund_bank }}</a-col>
      </a-row>
      <a-row class="mb5">
        <a-col :span="10">退款银行账号：</a-col>
        <a-col :span="12">{{ offLineInfo.refund_account }}</a-col>
      </a-row>
      <a-row class="mb5">
        <a-col :span="10">收款银行：</a-col>
        <a-col :span="12">{{ offLineInfo.receive_bank }}</a-col>
      </a-row>
      <a-row class="mb5">
        <a-col :span="10">收款银行账号：</a-col>
        <a-col :span="12">{{ offLineInfo.receive_account }}</a-col>
      </a-row>
