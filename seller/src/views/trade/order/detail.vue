<template>
  <div class="mr24">
    <a-card :bordered="false" title="订单进度">
      <div class="steps">
        ...
        <div class="fahuo">
          <a-button
            type="primary"
            @click="goDetail(detail.tid)"
            v-if="detail.status_text == '待发货' && detail.pick_type_name !=='自提'"
          >
            我要发货
          </a-button>
        </div>
      ...
      <div class="det-info">
        <a-row>
          ...
          <a-col :md="6">
            ...
            <p>应付金额：￥{{ detail.amount }}</p>
            <p>首单免邮：{{ detail.is_first_order === 1 ? '是' : '否' }}</p>
          </a-col>
          <a-col :md="6">
            <p>配送方式：{{ detail.pick_type_name }}</p>
            <p v-if="detail.pick_type && detail.cancel_text !== '系统关闭'">
              核销时间：{{  detail.end_time || "未核销" }}
            </p>
			<p>追加备注：{{ detail.add_remarks }}</p>
          </a-col>
        </a-row>
      </div>
    </a-card>
        ...
        <a-col :md="12" :xs="24">
          <div class="det-remark">
            <div class="buyer">
              ...
              <div class="buyer-info" v-if="detail.buyer_message">
            ...
            <div>
              ...
              <div class="mt10">
                <a-button
                  type="primary"
                  @click="shopRemarks(detail.tid)"
                  :disabled="formItem.remarks != '' ? false : true"
                  >保存</a-button
                >
              </div>
            </div>
    ...
    <a-card :bordered="false" title="物流信息" v-if="logSteps.length !== 0">
      <a-steps progressDot :current="0" direction="vertical">
        <a-step
          v-for="(item, index) in logSteps"
          :title="item.text"
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      logLists: state => state.trade.logLists
    })
  },
  ...
  methods: {
    ...mapActions({
      ...
      detailRemarks: "trade/detailRemarks",
      trade_log: "trade/trade_log"
    }),
    //门店备注
    shopRemarks(tid) {
      this.formItem.id = tid;
      this.detailRemarks(this.formItem);
    },
    initDet() {
      switch (this.detail.status_text) {
        ...
        case "待收货":
          this.stepCurrent = 2;
          this.trade_log(this.$route.params.id);
          break;
        case "已完成":
          this.stepCurrent = 3;
          break;
  ...
  watch: {
    detail() {
      this.initDet();
      this.formItem.remarks = this.detail.shop_memo;
      // console.log(this.data);
    },
    logLists() {
      this.logSteps = this.logLists;
    }
  }
};
</script>
