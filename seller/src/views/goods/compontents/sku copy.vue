  ...
  methods: {
    ...
    changeStock(option, index, stockCopy, lvIndex, sp_value) {
      ...
      if (option === "add") {
        ...
        if (this.stockSpecArr.findIndex(item => objEquals(spec, item)) === -1) {
          if (lvIndex === 0 || this.stockOne) {
            console.log("$set");
            this.$set(this.childProductArray, index, childProduct);
          } else {
            console.log("splice");
            this.childProductArray.splice(index, 0, childProduct);
          }
        }
      } else if (option === "del") {
    ...
    initSkuDetail(newName) {
      if (newName.sku !== undefined) {
        ...
        let specList_arr = [];
        this.specList.map((skuItem, index) => {
          specList_arr.push(skuItem.value);
        });
        let allArr = this.cartesianProductOf(...specList_arr);
        let allSku = [];
        allArr.map((item, index) => {
          newName.sku.map((v, i) => {
            console.log(v.sp_value)
            if (item.join("") === Object.values(v.sp_value).join("")) {
              allSku.push(v);
            }
          });
        });
        
        this.childProductArray = allSku;
        const lists = {
