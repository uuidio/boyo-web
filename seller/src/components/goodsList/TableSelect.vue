<template>
  <div>
    ...
    <a-modal
      ...
      @cancel="handleCancel"
    >
      <a-input-search v-if="showType!=='group'" enterButton class="mb10" style="width: 18rem;" placeholder="请输入名称或名称关键字搜索" @search="shopSearch"/>
    ...
    getLists(currentPage, searchVal) {
      fetch
        .get(this.newSelectionUrl, {
          params: {
            page: currentPage, keyword: this.keyword, shop_state: 1, goods_state: 1,
          },
        })
        .then((response) => {
          if (response.data.code === 0) {
            ...
            if (this.showType == 'group') {
              this.listField[6].scopedSlots = { customRender: 'image_url' };
