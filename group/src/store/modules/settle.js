...
const actions = {
  getDownloadUv({ commit }, params) {
    http
      .get(Server.action.tradeUvListsDown, params)
      .then((response) => {
        if (response.code === 0) {
          commit('setUvDownload', response.result);
        } else {
          helper.showError(response.message);
        }
      });
  },
  getTradeUv({ commit },params) {
    http
      .get(Server.action.tradeUv,params)
      .then((response) => {
        if (response.code === 0) {
          ...
          // });
          commit('setUvPages',response.result.lists);
          commit('setUvFields',fieldList);
          commit('setUvLists',response.result.lists.data);
        } else {
          helper.showError(response.message);
        }
      });
...
const mutations = {
  setUvDownload(state, lists) {
    state.uvDownload = lists;
  },
  setUvFields(state, lists) {
    state.uvFields = lists;
  },
  setUvLists(state, lists) {
    state.uvLists = lists;
  },
  setUvPages(state, lists) {
    state.uvPages = lists;
  },
