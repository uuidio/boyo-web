<template>
	...
	<div class="total"><span v-for="(item,index) in total_data" :key="index" class="span">{{item.title+': '}}<span class="text">{{item.value}}</span></span></div>
		<template v-if="listField&&listField.length>0">
			<a-tabs @tabClick="onGetData" :defaultActiveKey="trim">
				<a-tab-pane v-for="(item, index) in tabLists" :tab="item.tab" :key="item.key">
					<a-table :scroll="{ x: 2000,y:700 }"  :rowKey="record => record.id" :rowSelection="{onChange: onChange}" :columns="listField" :dataSource="listData" :loading="loadingState" :pagination="false">
        ...
        <a-form-item label="项目选择:" :label-col="{ md: 6,xs:24 }" :wrapper-col="{ md: 18,xs:24 }">
          <a-select v-model="exportForm.gm_id" placeholder="请选择项目" allowClear>
      ...
      setTimeout(() => {
		...
		dataHandleOk(){
			...
			if(Object.keys(this.exportForm).length == 0 ){
				...
				//this.exportFilter({ exportForm: this.exportForm,collback:this.orderCallback} ) // 请求数据
