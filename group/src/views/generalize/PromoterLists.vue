<template>
  <div class="content">
    <div>
      <div class="mb20">
        <a-button icon="arrow-down" type="primary" @click="handleOk">导出</a-button>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        listData: state => state.generalize.PromoterLists,
        listField: state => state.generalize.PromoterFields,
        pages: state => state.generalize.PromoterPages,
      }),
    },
    data() {
      return {
    ...
    methods: {
      ...mapActions({
        depLists: 'generalize/promoterLists',
        promoterDown: 'generalize/promoterDown',
      }),
      toFx(item){
