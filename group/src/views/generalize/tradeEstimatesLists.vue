...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        listData: state => state.generalize.TradeLists,
        listField: state => state.generalize.TradeFields,
        pages: state => state.generalize.TradePages,
      }),
    },
    data() {
      return {
    ...
    methods: {
      ...mapActions({
        depLists: 'generalize/tradeEstimatesLists',
      }),
      timeSearch(){
    ...
    mounted() {
      if(this.$route.params.id){
         this.listform.pid = this.$route.params.id
         this.depLists(this.listform)
